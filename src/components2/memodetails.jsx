...
function Memodetails() {
  ...
  const [updateId, setUpdateId] = useState("");
    ...
    const fetchData = async () => {
        try {
          const result = await axios.get(`http://localhost:4001/api/memodetails?search=${searched}`);
      ...
      const handleSubmit = async (value) => {
        if (updateId === "") {
          try {
            ...
            fetchData();
            notification.success({ message: "memodetails Added successfully" });
            setOpen(false);
          } catch (err) {
...
<div className="w-full flex gap-5 items-end justify-end">
        ...
        <Table columns={columns} dataSource={Memodetails}  ref={tableRef}/>
      </div>
      <Modal
        open={open}
        width={700}
